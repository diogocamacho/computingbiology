...
limma_dge <- function(expression_data,caseIds=list(),ctrIds)
{
  if(!is.list(caseIds)) #ie, only one case
  {
    ...
    contmat <- limma::makeContrasts(case-control,levels=expdes) # contrast matrix
    
    limma.fit <- limma::lmFit(tmp_data,design=expdes)
    limma.fit <- limma::contrasts.fit(limma.fit,contmat)
    limma.fit <- limma::eBayes(limma.fit)
    
    res <- limma::topTable(limma.fit,1,number=nrow(tmp_data),sort.by = "none",adjust.method = "fdr")
    return(res)
